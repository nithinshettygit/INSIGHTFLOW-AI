"""RAG Engine — PDF extract → chunk → embed → FAISS → answer."""
...
from app.engines.rag.chunking import TextChunk, chunk_pages
...
class RagEngine:
    """Document QA over uploaded PDFs using FAISS retrieval."""

    def __init__(
        self,
        settings: Settings | None = None,
        store: FaissChunkStore | None = None,
    ...
    def index_pdf(
        ...
        *,
        ...
        logger.info(
            "RAG indexed dataset_id=%s pages=%s chunks=%s",
            dataset_id,
            page_count,
            len(chunks),
        )
        ...
        *,
        ...
        if not self.store.exists(dataset_id):
            raise RagEngineError(
                "No RAG index for this dataset. Re-upload the PDF or POST /rag/index.",
                status_code=404,
            )
        ...
        if meta is None or meta.chunk_count == 0:
            raise RagEngineError(
                "PDF has no extractable text to search. "
                "Try a text-based PDF (not a scanned image-only document).",
            )

        k = top_k or self.settings.rag_top_k
        query_vec = self.embedder.embed_query(cleaned)
        hits = self.store.search(dataset_id, query_vec, k)
        sources = [
            {
        ...
        use_llm = self.settings.rag_use_llm if synthesize is None else synthesize
        if use_llm and self.settings.groq_api_key.strip():
            answer, provider = self._synthesize_with_llm(cleaned, sources)
        else:
            answer, provider = self._extractive_answer(cleaned, sources)
    ...
    def _synthesize_with_llm(
        ...
        try:
            ...
            messages = [
                ...
                HumanMessage(content=build_rag_user_prompt(question, sources)),
            ]
            ...
            logger.warning("RAG LLM synthesis failed, using extractive fallback: %s", exc)
            answer, _ = self._extractive_answer(question, sources)
    ...
    def _extractive_answer(
        ...
        if not sources:
            return (
                "No relevant passages were found in the indexed document.",
                "extractive",
            )
        top = sources[0]
        ...
        answer = (
            f"Based on the closest passage{page_bit} for “{question}”:\n\n{excerpt}"
        )
        if len(sources) > 1:
            answer += f"\n\n({len(sources) - 1} additional supporting passage(s) retrieved.)"
